...
const likedButtons = document.querySelectorAll('.like-button');
...
likedButtons.forEach((likedButton) => {
    likedButton.addEventListener('click', function(e) {
        ...
        if(!likedPosts.includes(idPost)) {
            ...
            const counterPostLikes = document.querySelector('#like-counter-' + idPost);

            const updatedLikes = parseInt(counterPostLikes.innerHTML);
            counterPostLikes.innerHTML = updatedLikes + 1;
        };
    });
});